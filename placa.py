...
def process_files_in_folder(folder_path):
    ...
    # Gerar arquivos PKL para veículos que passaram pelo menos duas vezes
    for placa, count in vehicle_counts.items():
        if count >= 2:
            vehicle_data = pd.concat([df[df['placa_veiculo'] == placa] for df in filtered_dfs])
            output_file_path = os.path.join(output_folder_path, f'{placa}_passagens.pkl')
            vehicle_data.to_pickle(output_file_path)
            print(f"Arquivo gerado para o veículo {placa}: {output_file_path}")
