...
pkl_file = 'veiculos_31kmh_550_veiculos.pkl'  # Substitua pelo nome do arquivo desejado
    ...
    if df.empty:
        ...
        for index, row in df.iterrows():
            ...
            if len(peaks0) > 0 or len(peaks1) > 0 or len(peaks2) > 0:
                ...
                plt.scatter(x=peaks0, y=np.array(data['FBG_0'])[peaks0], color='black', zorder=5, label='Picos FBG_0')
                plt.plot(data['FBG_1'], label='FBG_1', color='green')
                plt.scatter(x=peaks1, y=np.array(data['FBG_1'])[peaks1], color='black', zorder=5, label='Picos FBG_1')
                plt.plot(data['FBG_2'], label='FBG_2', color='red')
                plt.scatter(x=peaks2, y=np.array(data['FBG_2'])[peaks2], color='black', zorder=5, label='Picos FBG_2')
